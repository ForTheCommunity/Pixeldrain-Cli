...
impl Default for UploadPipelineOptions {
    fn default() -> Self {
        Self {
            ...
            formats: None,
            delete_after: false,
            max_concurrent_small_uploads: 4,
        }
    }
}
...
impl UploadPipeline {
    ...
    pub async fn run(self) -> Result<()> {
        ...
        if !large_files.is_empty() {
            let responses = self.process_lfs(large_files, api_client, &progress).await?;
    ...
    // uploads small files simaltaneously.
    ...
    // uploads large files, 1 by 1, with per-byte progress tracking.
